...
public final class Amend extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        new UpdateChecker(this, UpdateCheckSource.CUSTOM_URL, "https://api.tronmc.com/amend/versions/1.21.3") // A link to a URL that contains the latest version as String
    ...
    public void onDisable() {
        ...
        if (MCVersion.equals("1.21.3")) {

            if (ServerType.equals("paper")) {
                URLConnection connection = null;
                try {
                    connection = new URL("https://api.tronmc.com/amend/versions/paper/1.21.3").openConnection();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                try (Scanner scanner = new Scanner(connection.getInputStream())) {
                    ...
                    if (version != latest) {
                        ...
                        InputStream in = new URL("https://api.tronmc.com/amend/versions/paper/1.21.3/download/direct").openStream();
                ...
                URLConnection connection = null;
                try {
                    connection = new URL("https://api.tronmc.com/amend/versions/purpur/1.21.3").openConnection();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                try (Scanner scanner = new Scanner(connection.getInputStream())) {
                    ...
                    if (version != latest) {
                        ...
                        getLogger().info("Downloading update and applying to " + serverJarName + "...");
                        InputStream in = new URL("https://api.tronmc.com/amend/versions/purpur/1.21.3/download/direct").openStream();
                        Files.copy(in, Paths.get(serverJarName), StandardCopyOption.REPLACE_EXISTING);
                        getLogger().info("Update Completed!");
                        getLogger().warning("-------------------------------");
                        try {
