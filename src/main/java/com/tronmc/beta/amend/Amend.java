...
public final class Amend extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        new UpdateChecker(this, UpdateCheckSource.CUSTOM_URL, "https://api.tronmc.com/amend/versions/1.20.1") // A link to a URL that contains the latest version as String
    ...
    public void onDisable() {
        ...
        if (BukkitVersion.substring(start, end).equals("1.20.1")) {

            if (ServerType.equals("paper")) {
                URLConnection connection = null;
                try {
                    connection = new URL("https://api.tronmc.com/amend/versions/paper/1.20.1").openConnection();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                try (Scanner scanner = new Scanner(connection.getInputStream())) {
                    ...
                    if (version != latest) {
                        ...
                        InputStream in = new URL("https://api.papermc.io/v2/projects/paper/versions/1.20.1/builds/" + response + "/downloads/paper-1.20.1-" + response + ".jar").openStream();
                ...
                URLConnection connection = null;
                try {
                    connection = new URL("https://api.purpurmc.org/v2/purpur/1.20.1").openConnection();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                try (Scanner scanner = new Scanner(connection.getInputStream())) {
                    ...
                    if (version != latest) {
                        ...
                        getLogger().info("Downloading update and applying to " + serverJarName + "...");
                        InputStream in = new URL("https://api.purpurmc.org/v2/purpur/1.20.1/latest/download").openStream();
                        Files.copy(in, Paths.get(serverJarName), StandardCopyOption.REPLACE_EXISTING);
                        getLogger().info("Update Completed!");
                        getLogger().warning("-------------------------------");
                        try {
            ...
            getLogger().warning("Current Version: " + BukkitVersion.substring(11));
            getLogger().warning("Plugin Version: " + "1.20.1");
            getLogger().warning("Closing plugin...");
            getLogger().warning("-------------------------------");
            //Time out so user sees the error message.
            try {
